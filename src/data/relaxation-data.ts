...
export const relaxationSteps: RelaxationStep[] = [
  {
    title: "Подготовка",
...
export const relaxationFAQs: FAQItem[] = [
  {
    ...
    answer: "Для достижения наилучших результатов рекомендуется практиковать технику ежедневно, хотя бы один раз в день. По мере освоения метода, эффект расслабления будет наступать быстрее, и вы сможете применять его в любых стрессовых ситуациях."
  },
  {
    question: "Сколько времени занимает полная процедура?",
    answer: "Полная процедура прогрессивной мышечной релаксации обычно занимает 15-20 минут. С опытом вы можете сократить это время до 5-7 минут, фокусируясь только на тех группах мышц, где чаще всего ощущаете напряжение."
  },
  {
    question: "Могут ли быть противопоказания к этой технике?",
    answer: "Техника обычно безопасна для большинства людей, но если у вас есть травмы мышц, проблемы с суставами или какие-либо другие физические ограничения, следует проконсультироваться с врачом перед началом практики. Также стоит быть осторожными при напряжении определенных групп мышц, если у вас есть проблемы с сердцем или высокое кровяное давление."
  },
  {
    question: "Можно ли использовать эту технику детям?",
    answer: "Да, прогрессивная мышечная релаксация может быть адаптирована для детей. Для младших детей можно представить упражнения в форме игры, например, \"сожми лимон\" для рук или \"встань на цыпочки как жираф\" для ног. Важно делать упражнения короче и использовать образные сравнения, понятные ребенку."
  },
  {
    question: "Как понять, что я правильно выполняю технику?",
    answer: "При правильном выполнении вы должны чувствовать явный контраст между напряжением и расслаблением мышц. После полной сессии вы можете ощущать приятную тяжесть в теле, чувство тепла в мышцах и общее снижение тревоги. Важно не перенапрягать мышцы — напряжение должно быть умеренным, без боли или дискомфорта."
  }
];
...
export const relatedTechniques: RelatedTechnique[] = [
  ...
  },
  {
    ...
    description: "Использование силы воображения для создания спокойствия и гармонии.",
    image: "https://images.unsplash.com/photo-1557672172-298e090bd0f1?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=500&q=80",
    icon: "Star"
  }
];
