...
const SelfAssessment: React.FC = () => {
  ...
  const [currentStep, setCurrentStep] = useState<'select' | 'test' | 'results'>('select');
  ...
  const calculateMaxScore = (): number => {
    ...
    const maxFromResults = Math.max(...selectedTest.results.map(r => r.maxScore));
          ...
          <div ref={testSectionRef} className="mb-12">
            <div className="bg-white rounded-xl shadow-lg p-8 mb-6">
              ...
              <TestQuestion 
                ...
                currentAnswer={answers[selectedTest.questions[currentQuestionIndex].id] || null}
            ...
            <div className="bg-[#E5DEFF] p-4 rounded-lg">
              <div className="flex items-start">
                <Icon name="Info" className="h-5 w-5 text-[#9b87f5] mr-2 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm">
                    <span className="font-medium">Автор теста:</span> {selectedTest.author}, {selectedTest.year}
                  </p>
                  <p className="text-sm mt-1">
                    Для получения наиболее точных результатов, пожалуйста, отвечайте честно и без длительных размышлений.
                  </p>
                </div>
              </div>
            </div>
        ...
        {currentStep === 'results' && selectedTest && testResult && (
          <div ref={testSectionRef} className="mb-12">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h3 className="text-lg font-semibold mb-3">Источники и литература:</h3>
